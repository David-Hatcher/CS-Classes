...
###
#buildKeyWordSet creates the unique list of keywords
###
...
def addValue(hashTable,word_item,tSize,gValues,backtrack):
    maxAttempts = int(tSize/2)
    print(word_item," :: ",hashTable, end='\n\n')
    i = 0
    word = word_item[0]
    length = word_item[2]
    if(backtrack == False):
...
def buildHashTable(words,lettersCount):
    ...
    i = 0
    while i < len(words):
        if(addValue(hashTable,words[i],len(words),gValues,backtrack)[0] == False):
            i -= 1
