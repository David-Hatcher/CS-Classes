...
void sortProducts(struct product products[], int n,int maxQuantity);
...
int main(){
    ...
    sortProducts(products,0,numValues);
...
void sortProducts(struct product products[], int n,int maxQuantity){
    int i, currentIMax = n;
    struct product currentMax = products[n];
    for(i = n+1; i < maxQuantity; i++){
        if(products[i].quantity > currentMax.quantity){
            currentMax = products[i];
            currentIMax = i;
        }
    }
    struct product temp = products[n];
    products[n] = products[currentIMax];
    products[currentIMax] = temp;
    if(n+1 <= MAX_QUANTITY){
        sortProducts(products,n+1,maxQuantity);      
    }
}
