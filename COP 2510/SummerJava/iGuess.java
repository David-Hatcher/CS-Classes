...
public class iGuess {
  private static ArrayList<ArrayList<String>> combos = new ArrayList<ArrayList<String>>();
  private static ArrayList<String> secretCode = new ArrayList<String>(); 
  ...
  public static ArrayList<String> generateSecretCode(){
    secretCode.clear();
    ArrayList<String> choices = new ArrayList<String>(Arrays.asList(possibles));
    int spaceForAt = rand.nextInt(5);
    for(int i = 0; i < 5; i++){
      if(spaceForAt == i){
        secretCode.add("@");
      }
      else{
        secretCode.add(choices.remove(rand.nextInt(choices.size())));
      }
    }        
    return secretCode;
  }
