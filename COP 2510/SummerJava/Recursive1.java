...
public class Recursive1 {
    ...
    public static ArrayList<ArrayList<String>> combosNoDupe = new ArrayList<ArrayList<String>>();
    ...
    public static boolean checkForDuplication(ArrayList<ArrayList<String>> possiblesArrayList) {
        boolean doesListMatch = true;
        for (ArrayList<String> combo : possiblesArrayList) {
            if(!combos.contains(combo)){
                doesListMatch = false;
            }
        }
        return doesListMatch;
    }

    public static void main(String[] args) throws IOException {
        long start = System.nanoTime();
        String[] letters = {"v", "w", "x", "y", "z", "5", "6", "7", "8", "9", "=", "?", "%", "$"};
        ArrayList<ArrayList<String>> possibles = iGuess.readFile("Possibles.txt");
        createLists(letters,5);
        System.out.println("Combos size = " + combos.size());
        boolean doesListMatch = checkForDuplication(possibles);
        System.out.println("Does List match = " + doesListMatch);
