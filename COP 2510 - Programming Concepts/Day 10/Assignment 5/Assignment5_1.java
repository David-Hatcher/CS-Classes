public class Assignment5_1{
  ...
  private static void printTriangle(String[][] triangle, String methNum){
    print("\ntriangle_" + methNum + "\n\n");
  ...
  private static void triangle_0(int totalLines){
    String[][] triangle = new String[totalLines][totalLines];
  ...
  private static void triangle_2(int totalLines){
    ...
    for(int i = 0; i < totalLines; i++){
      for(int j = 0; j < totalLines; j++){
        if(i <= j){
          triangle[i][j] = Integer.toString(totalLines- i);
        }
      }
    }
  ...
  private static void triangle_3(int totalLines){
    ...
    for(int i = 0; i < totalLines; i++){
      for(int j = 0; j < totalLines; j++){
        if(i <= j){
          triangle[i][j] = Integer.toString(totalLines - j);
        } 
      }
    }
  ...
  private static void triangle_5(int totalLines){
    ...
    while(i < totalLines){
      ...
      while(j < totalLines){
        if(i >= j){
          triangle[i][j] = Integer.toString(i+1);
        }
        j++;
      }
      i++;
  ...
  private static void triangle_6(int totalLines){
    ...
    while(i < totalLines){
      ...
      while(j < totalLines){
        if(i >= j){
          triangle[i][j] = Integer.toString(j+1);
        }
        j++;
      }
      i++;
