...
void Office::readDescriptions(){
    this->mirror_broken_desc = readFile("Rooms/" + this->file_path + "/" + this->file_path + "mirrorbrokendesc.txt");
    this->mirror_not_broken_desc = readFile("Rooms/" + this->file_path + "/" + this->file_path + "mirrornotbrokendesc.txt");

}
std::string Office::breakMirror(Player& player){
    Item hammer = buildItem("Hammer");
...
std::string Office::touchMirror(Player& player){
    Item mirror_key = buildItem("Mirror Key");
    if(player.checkFlags("Mirror_broken")){
